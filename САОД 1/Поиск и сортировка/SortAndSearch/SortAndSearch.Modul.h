...
/// Средний: O(n^2)
/// Худший: O(n^2) - массив отсортирован по убыванию
template <typename T>
...
void quicksort(vector<T>& arr, size_t begin, size_t end) {
	if (begin >= end) return; // <= БСР (начальный индекс больше или равен конечному)
...
/// Базовый случай рекурсии - это условие при котором рекурсия останавливается
